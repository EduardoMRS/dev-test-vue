...
# - Sem npm install / vite build (feitos no Dockerfile.prod)
# - Sem Vite dev server
# - Config/route/view cache ativados
...
if [ ! -f .env ]; then
  if [ -f .env.example.production ]; then
    echo "No .env found — copying .env.example.production"
    cp .env.example.production .env
  elif [ -f .env.example ]; then
    echo "No .env found — copying .env.example"
    cp .env.example .env
  else
    echo "No .env found and no example file available. Please provide a .env file."
    exit 1
  fi
fi
...
if php artisan list --raw 2>/dev/null | grep -q '^vapid:generate'; then
  ...
  echo "vapid:generate command not available; skipping VAPID key generation"
fi
...
# A porta interna do container é SEMPRE 80.
# A porta do host é definida pelo port binding no docker-compose (APP_PORT).
