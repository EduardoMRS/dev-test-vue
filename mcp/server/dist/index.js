...
function readProjectFile(relativePath) {
    ...
    if (stat.size > 500_000)
        return `[Arquivo muito grande para leitura inline: ${relativePath} (${stat.size} bytes)]`;
    return fs.readFileSync(abs, 'utf-8');
}
function searchCode(pattern, dir = '.', ext = '') {
    try {
...
function listPhpFiles(dir) {
    ...
    for (const entry of fs.readdirSync(abs, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            results.push(...listPhpFiles(full));
...
// ─── Server definition ────────────────────────────────────────────────────────
const server = new Server({ name: 'laravel-vue-template', version: '1.0.0' }, { capabilities: { tools: {} } });
// ─── List available tools ─────────────────────────────────────────────────────
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: 'project_overview',
            description: 'Retorna a visão geral do projeto: stack, estrutura de pastas, convenções e fluxo de autenticação.',
        ...
        {
            name: 'get_auth_flow',
            description: 'Descreve o fluxo completo de autenticação: Fortify (sessão) + Sanctum (Bearer token) + SPA.',
            inputSchema: { type: 'object', properties: {}, required: [] },
        },
        {
            name: 'read_file',
